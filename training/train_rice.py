...
def quantise(model, classes: list[str]) -> tuple[bytes, str]:
    ...
    """
    calib = sorted((PREPARED / "cross_test").rglob("*.jpg"))[:200]
...
def main() -> None:
    ...
    checkpoint = pathlib.Path("best.keras")
    model.fit(
    ...
    artifact = OUT_DIR / "rice_disease_v1.tflite"
    artifact.write_bytes(tflite)
    (OUT_DIR / "rice_disease_v1.labels.json").write_text(json.dumps(classes, indent=2))
    ...
    }, open(OUT_DIR / "rice_disease_v1.metrics.json", "w"), indent=2)
