...
BATCH = 32
SEED = 1337
PREPARED = pathlib.Path("data/prepared_multi")
...
def quantise(model, classes: list[str]) -> tuple[bytes, str]:
    ...
    """
    calib = sorted((PREPARED / "holdout").rglob("*.jpg"))[:200]
...
def main() -> None:
    ...
    checkpoint = pathlib.Path("best_v2.keras")
    model.fit(
    ...
    print(f"THIRD-collection holdout    : {cross['accuracy']:.4f} over {cross['images']} images")
    for name, acc in cross["per_class_accuracy"].items():
        print(f"   {name:<34} {acc:.3f}")

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    tflite, precision = quantise(model, classes)
    artifact = OUT_DIR / "rice_disease_v2.tflite"
    artifact.write_bytes(tflite)
    (OUT_DIR / "rice_disease_v2.labels.json").write_text(json.dumps(classes, indent=2))
    ...
    }, open(OUT_DIR / "rice_disease_v2.metrics.json", "w"), indent=2)
